...
//        return (flightSection) -> flightSection.getRemainingVotes() >= remainVotes;
    ...
    public List<FlyPlan> mutiSearch(final FlightSearchDTO flightArgs) {
//        FlightStrategy strategy = StrategyBuilder.flightStrategyMap.get(FlightStrategy.STRATEGY.DIRECT);
//        List<List<FlyPlan>> collectEachWay = flightArgs.getSearchList().stream()
//                .map(toSearch ->
//                        strategy.strategy(
//                                airportService.searchAirport(toSearch.getSourcePlace()),
//                                airportService.searchAirport(toSearch.getDestinationPlace()),
//                                toSearch.getStartDate(), null
//                        ).stream().limit(5).collect(Collectors.toList())
//                ).collect(Collectors.toList());
//        collectEachWay.get(0).forEach(firstPlan->{
//            collectEachWay.stream().skip(1).forEach(afterPlan->{
//                firstPlan.joinPlanRtn(afterPlan.get());
//            });
//        });
//        collectEachWay.forEach(flyPlans -> {
//            collectEachWay.stream().skip(1).forEach(afterPlan->{
//
//            });
//        });
